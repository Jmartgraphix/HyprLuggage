...
clean_shell() {
    ...
    for rc in ~/.bashrc ~/.zshrc ~/.config/fish/config.fish; do
        [[ -f "$rc" && ! -L "$rc" ]] || continue
        grep -q "[Hh]yprluggage" "$rc" 2>/dev/null || continue
        sed -i '/[Hh]yprluggage/d;/install\/themes/d' "$rc"
        ok "$(basename "$rc")"
    done
    return 0
}
...
clean_desktop_entries() {
    ...
    # Collect entries installed by Hyprluggage
    local entries=()
